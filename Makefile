...
build/boost_unordered_map: src/boost_unordered_map.cc Makefile src/template.c
	g++ -O2 -lm -I/usr/local/include src/boost_unordered_map.cc -o build/boost_unordered_map

build/google_sparse_hash_map: src/google_sparse_hash_map.cc Makefile src/template.c
	g++ -O2 -lm -I/usr/local/include src/google_sparse_hash_map.cc -o build/google_sparse_hash_map

build/google_dense_hash_map: src/google_dense_hash_map.cc Makefile src/template.c
	g++ -O2 -lm -I/usr/local/include src/google_dense_hash_map.cc -o build/google_dense_hash_map

build/qt_qhash: src/qt_qhash.cc Makefile src/template.c
	g++ -O2 -lm -I/usr/local/include `pkg-config --cflags --libs QtCore` src/qt_qhash.cc -o build/qt_qhash
...
build/myhashmap: src/myhashmap.cc Makefile src/template.c
	g++ -O2 -lm -std=c++11 -I/usr/local/include src/myhashmap.cc -o build/myhashmap
